...
public class Logged extends NotLogged{
	...
	public void selectSeat(Movie movie, int postion) {
		...
		changeSeatsStatus(seats, selectedSeats, 1); // 修改座位状态
		movie.setSeats(seats); //写入新座位信息
		System.out.println("选择座位");
	}
	
	private void changeSeatsStatus(int[][] seats, List<List<Integer>> seatIndex, int status) {
